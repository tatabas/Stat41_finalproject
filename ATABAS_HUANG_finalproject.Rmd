...
Aside from the various scatterplots, it is interesting to see that a lot of the variables have bimodal distributions. Since the plots come from a long and versatile timeframe (an entire century), this may be an effect of changing taste over time. Changes in music usually reflects society's expressed mood (both in terms of production and consumption), so this is not a surprise. This is further supported by Figure \@ref(fig:scattervalyear), since the valence (or happiness) values fluctuate over time. This is especially clear in the transition from the "roaring 20s" to the 40s and 50s. The detrimental effects of World War II can even be seen in music! 
...
Looking at this data split by region allows for us to see that not every region is the same, and that there are different trends. However, this idea also exists in specific countries of a given region. When viewed as a whole as in Figure \@ref(fig:not-region), the global trends on assault and robbery do not have much of a change, but there is an increasing trend in sexual violence. It may be true that sexual violence is increasing, but it may also be true that it is getting reported more. This may be in changes with the laws, or re-classifying types of assaults as sexual violence. In any way, there is an increase in sexual violence crimes according to the UNODC data.

```{r not-region, fig.width = 9, fig.height = 3, fig.cap = "Rates of Crime by Type and Year}
p1 <- assault %>%
  group_by(Year, Region) %>%
  summarise(
    means = mean(as.numeric(Rate), na.rm = TRUE)
...
```{r happy-box, fig.width=5, fig.height=3, fig.cap="Box plot of valence scores overlayed with points from the dataset"}
spotify_by_year_small <- spotify_by_year %>% filter(year == 2015 | year == 2016 | year == 2017 | year == 2018 | year == 2019 | year == 2020)
...
```{r}
spotify_happiness_lm <- lm(mean ~ valence, data = spotify_and_happiness)
summary(spotify_happiness_lm)
```
...
```{r barcrime, fig.height=3, fig.width=5, fig.cap="Percent changes in explicit and non-explicit music over time"}
library(ggtext)
spotify %>%
  mutate (
...
```{r regress-assault, fig.height=3, fig.width=5, fig.cap="Comparing rate of crime (for 100,000 population) and percent explicit music per year for 2010-2017, displayed by region for each type of crime: assault, robbery, and sexual violence"}
all_together %>%
