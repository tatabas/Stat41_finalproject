...
#data is cleaned in ATABAS_HUANG_dataCleaning.Rmd
...
(Note: loading all data from ATABAS_HUANG_loadData.Rmd)
...
```{r}
  ggplot(data = spotify_by_year,aes(x = acousticness, y = valence)) + 
              geom_point()+
          theme_proj +
          #need to add gridlines because tufte removes them
          theme(panel.grid.major = element_line(color = "gray90"),
...
server <- function(input, output) {
    ...
    output$sub <- renderUI({
        ...
        selectInput(inputId = "sub", label = "Sub Region:", choices = sub_this_location, selected = "Eastern Africa")
    })
...
p1 <- assault %>%
  ...
  labs(x = "Year", y = "Mean Rate per 100,000 population",
       ...
       caption = "Source: UNODC Serious Assault Data") +
...
p3 <- sexualviolence %>%
  ...
  labs(x = "Year", y = element_blank(),
       ...
       caption = "Source: UNODC Sexual Violence Data") +
  theme(plot.title = element_text(size = 8), 
...
```{r}
comparing_assault <- spotify %>%
  filter(year >= 2010 & year <= 2017) %>%
  inner_join(., assault, by = c("year" = "Year")) %>%
  mutate(
    type = "Assault"
  )

comparing_robbery <- spotify %>%
  filter(year >= 2010 & year <= 2017) %>%
  inner_join(., robbery, by = c("year" = "Year")) %>%
  mutate(
    type = "Robbery"
  )

comparing_sexualviolence <- spotify %>%
  filter(year >= 2010 & year <= 2017) %>%
  inner_join(., sexualviolence, by = c("year" = "Year")) %>%
  mutate(
...
```{r crimeandexplicitness, fig.width = 9, fig.height= 3, fig.cap = "Logistic regression on rates of crime and explicitness of songs by region for the years 2010 - 2017"}
c1 <- ggplot(data = comparing_assault, aes(x = Rate, y = explicit)) +
  geom_jitter(width = 0.3, height = 0.05, alpha = 0.3, size = 0.05) +
  geom_smooth(
...
c2 <- ggplot(data = comparing_robbery, aes(x = Rate, y = explicit)) +
  geom_jitter(width = 0.3, height = 0.1, alpha = 0.3, size = 0.05) +
  geom_smooth(
    method = "glm", method.args = list(family = "binomial"), 
    color = "dodgerblue", se = FALSE
  ) +
...
c3 <- ggplot(data = comparing_sexualviolence, aes(x = Rate, y = explicit)) +
  geom_jitter(width = 0.3, height = 0.05, alpha = 0.3, size = 0.05) +
  geom_smooth(
    method = "glm", method.args = list(family = "binomial"), 
    color = "dodgerblue", se = FALSE
...
```
```{r}
ggplot(data = all_together, aes(x = Rate, y = explicit)) +
  geom_jitter(width = 0.3, height = 0.05, alpha = 0, size = 0.05) +
  geom_smooth(
    method = "glm", method.args = list(family = "binomial"), 
    color = "dodgerblue", se = FALSE, size = 1
  ) +
  facet_grid(type ~ Region, scale = "free_x") +
  theme_proj
```

# Predicting the Next Popular Song!

(Here, we plan on using a multiple regression to try and predict what attributes the next popular song will have. We're unsure how to visualize this, but we know you can fit a multiple linear regression to it.)
