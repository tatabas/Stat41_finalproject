...
comparing_assault <- spotify2 %>%
  filter(year >= 2010 & year <= 2017) %>%
  inner_join(., subset(assault, select = c(-Subregion, -Country)) %>% group_by(Region), by = c("year" = "Year")) %>%
  mutate(
    type = "Assault"
  )
...
comparing_robbery <- spotify2 %>%
  filter(year >= 2010 & year <= 2017) %>%
  inner_join(., subset(robbery, select = c(-Subregion, -Country)), by = c("year" = "Year")) %>%
  mutate(
    type = "Robbery"
  )
...
comparing_sexualviolence <- spotify2 %>%
  filter(year >= 2010 & year <= 2017) %>%
  inner_join(., subset(sexualviolence, select = c(-Subregion, -Country)), by = c("year" = "Year")) %>%
  mutate(
